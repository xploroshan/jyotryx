...
export default function TraditionDashboard({ traditionId }: { traditionId: TraditionId }) {
  ...
  return (
    <PageTransition className="mx-auto max-w-6xl px-5 sm:px-8 py-10 sm:py-14">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-surface-50 border border-surface-900/[0.08] shadow-warm-sm px-8 sm:px-12 py-12 sm:py-14 mb-12">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-8">
          <div className="shrink-0 grid place-items-center w-28 h-28 rounded-3xl bg-primary-100/70 border border-primary-500/25 text-primary-700">
            <TraditionGlyph id={traditionId} size={64} weight={1.4} />
          </div>
          <div className="flex-1 min-w-0">
            <span className="inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full bg-primary-100/70 border border-primary-500/30 text-primary-700 uppercase tracking-[0.18em] mb-4">
              {name}
            </span>
            <h1
      ...
      {/* Feature tiles */}
      <section>
        <h2 className="text-[12px] uppercase tracking-[0.22em] text-primary-700 font-medium mb-6">
          {exploreCta}
        </h2>
        <Stagger.Container className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {cfg.features.map((f) => {
            ...
            const tile = (
              <div
                className={`group rounded-2xl bg-surface-50 border border-surface-900/[0.08] shadow-warm-sm p-6 h-full flex items-center gap-4 transition-all duration-300 ${
                  f.available
                    ? 'hover:-translate-y-1 hover:shadow-warm-lg hover:border-surface-900/[0.14]'
                    : 'opacity-50 cursor-not-allowed'
                }`}
              >
                <div className="shrink-0 grid place-items-center w-12 h-12 rounded-xl bg-primary-100/60 border border-primary-500/15 text-primary-700 group-hover:bg-primary-100 group-hover:border-primary-500/35 transition-colors">
                  <FeatureGlyph slug={f.slug} size={22} />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="font-display text-base sm:text-lg font-semibold text-surface-900 leading-tight">
                    {label}
                  </h3>
                  {!f.available && (
                    <p className="mt-1 text-xs text-surface-900/55">
