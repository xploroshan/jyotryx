      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 py-12 fade-in-up">
        ...
        {error && (
          <div className="text-center py-12">
            ...
            <button onClick={fetchPanchang} className="mt-4 px-6 py-2 rounded-xl btn-secondary text-sm text-primary-600 hover:bg-surface-900/[0.1]">
              {t.panchang.retry}
            </button>
          </div>
        )}
        ...
        {panchang && !loading && (
          <>
            ...
            <div className="surface-card p-6 mb-8 text-center">
              <p className="text-sm text-surface-900/30 mb-1">{t.panchang.date}</p>
              ...
              {panchangItems.map((item) => (
                <div key={item.label} className="surface-card p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <p className="text-xs text-surface-900/30">{item.desc}</p>
                      <p className="text-sm font-medium text-surface-900/40">{item.label}</p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              {timings.map((tm) => (
                <div key={tm.label} className="surface-card p-5 text-center">
                  ...
                  <p className="text-xs text-surface-900/30 mb-1">{tm.label}</p>
                  <p className="text-lg font-bold text-surface-900">{tm.value}</p>
                </div>
              ))}
            </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {inauspicious.map((item) => (
                <div key={item.label} className="surface-card p-5 border border-red-500/10">
                  ...
                  <p className="text-lg font-bold text-surface-900 mb-1">{item.value}</p>
                  <p className="text-xs text-surface-900/30">{item.desc}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-8 surface-card p-4 text-sm text-surface-900/60 flex items-center justify-between gap-4">
              <span>Want today's panchang for a specific city?</span>
              <Link
                href="/panchang/cities"
                className="text-primary-700 hover:text-primary-700 whitespace-nowrap"
              >
                Browse cities →
              </Link>
            </div>
