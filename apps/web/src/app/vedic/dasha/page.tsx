      ...
      >
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-surface-50/50 leading-relaxed">
              ...
              >
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-surface-50/40 mt-1">
                        {fmtDate(d.startDate)} &rarr; {fmtDate(d.endDate)}
                      </div>
                    </div>
                  </div>
                  {d.subPeriods && d.subPeriods.length > 0 && (
                    <span className="text-surface-50/30 text-xs shrink-0" aria-hidden>
                      {isOpen ? '▲' : '▼'}
                    </span>
                  )}
                </button>
                {isOpen && d.subPeriods && d.subPeriods.length > 0 && (
                  <div className="border-t border-white/[0.04] bg-white/[0.01] px-6 py-4 space-y-2">
                    <p className="text-[11px] uppercase tracking-widest text-surface-50/30 mb-3 font-medium">
                      {fp.antardasha}
                    </p>
                    {d.subPeriods.map((sp, j) => (
                      ...
                      >
                        <span className="text-surface-50/60 flex items-center gap-2">
                          <span aria-hidden>{PLANET_EMOJI[sp.planet] ?? '·'}</span>
                          {sp.planet}
                        </span>
                        <span className="text-surface-50/30">
