...
export default function PricingClient({
  ...
  /**
   * Server-fetched /payments/pricing settings. When present the plan cards
   * render from it in the initial server HTML (no skeleton flash, no XHR);
   * the Service/Offer JSON-LD for these prices is emitted by the SERVER
   * wrapper in page.tsx — do not re-add it here (duplicate schema).
   */
  initialPricing?: Record<string, string> | null;
}) {
  ...
  const [settings, setSettings] = useState<Record<string, string> | null>(initialPricing ?? null);
  const [plans, setPlans] = useState<Plan[] | null>(initialPricing ? buildPlans(initialPricing) : null);
  const [creditPacks, setCreditPacks] = useState<CreditPack[] | null>(
    initialPricing ? buildCreditPacks(initialPricing) : null,
  );
  ...
  const [pricingEnabled, setPricingEnabled] = useState<boolean | null>(
    initialPricing ? initialPricing["feature.pricing_page_enabled"] === "true" : null,
  );
  ...
  const [creditsEnabled, setCreditsEnabled] = useState<boolean>(
    initialPricing ? initialPricing["feature.credits_enabled"] !== "false" : true,
  );
  ...
  useEffect(() => {
    // Prices are locale-independent; only the plan-card labels are not. With
    // settings already in hand (server-seeded or previously fetched) a locale
    // change just rebuilds the cards locally — no XHR, no skeleton flash.
    if (settings) {
    ...
    api.get<Record<string, string>>("/payments/pricing")
      .then((s) => {
        if (cancelled) return;
        setSettings(s);
        setPricingEnabled(s["feature.pricing_page_enabled"] === "true");
        setCreditsEnabled(s["feature.credits_enabled"] !== "false");
        setPlans(buildPlans(s));
        setCreditPacks(buildCreditPacks(s));
      })
      .catch(() => {
        if (cancelled) return;
