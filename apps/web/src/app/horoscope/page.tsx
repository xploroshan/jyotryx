      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-12 fade-in-up">
        ...
        {isMultiTradition && (
          <div className="flex justify-center mb-8">
            <div className="inline-flex gap-1 rounded-xl bg-surface-900/[0.03] p-1">
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                      isActive
                        ...
                        : "text-surface-900/40 hover:text-surface-900/70"
                    }`}
                  >
          ...
          <>
              ...
              {signList.map((z) => (
                <button
                  ...
                  className={`flex flex-col items-center py-3 px-2 rounded-xl transition-all ${
                    selectedSign === z.id
                      ? "surface-card bg-surface-900/[0.06] border-primary-500/50"
                      : "hover:bg-surface-900/[0.03]"
                  }`}
                >
                  <span className="text-2xl mb-1">{z.symbol}</span>
                  <span className={`text-xs font-medium ${selectedSign === z.id ? "text-surface-900" : "text-surface-900/40"}`}>
                    {z.name}
                  </span>
                </button>
              ))}
            ...
            <div className="surface-card p-6 mb-8 flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="text-center sm:text-left">
                <h2 className="text-xl font-bold text-surface-900">{currentSign.name}</h2>
                <div className="flex flex-wrap gap-3 mt-1 justify-center sm:justify-start">
                  {isChinese ? (
                    <>
                      <span className="text-sm text-surface-900/40">{(currentSign as typeof CHINESE_ANIMALS[0]).years}</span>
                    ...
                    <>
                      <span className="text-sm text-surface-900/40">{(currentSign as typeof zodiacSigns[0]).date}</span>
              ...
              <div className="sm:ml-auto">
                {/* Period Tabs */}
                <div className="flex gap-1 rounded-xl bg-surface-900/[0.03] p-1">
                  {periods.map((p) => (
                    <button
                      key={p.id}
                      ...
                      className={`px-4 py-2 rounded-lg text-xs font-medium transition-all ${
                        selectedPeriod === p.id
                          ? "btn-primary text-surface-900"
                          : "text-surface-900/40 hover:text-surface-900"
                      }`}
                    >
                      {p.label}
                    </button>
        ...
        {error && !loading && (
          <div className="text-center py-12">
            ...
            <br />
            <button onClick={showSummary ? fetchMultiTradition : fetchHoroscope} className="mt-4 px-6 py-2 rounded-xl btn-secondary text-sm text-primary-600">
              {t.horoscope.retry}
            </button>
          </div>
        )}
        ...
        {horoscope && !loading && !error && !showSummary && (
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 surface-card p-6">
              ...
              <p className="text-surface-900/60 leading-relaxed">{horoscope.overview}</p>
            </div>
            ...
            <div className="surface-card p-6">
              ...
              <div className="space-y-4">
                <div className="p-3 rounded-xl bg-surface-900/[0.03]">
                  <p className="text-xs text-surface-900/30 mb-1">{t.horoscope.luckyNumbers}</p>
                  <p className="text-surface-900 font-semibold">{horoscope.lucky.number}</p>
                </div>
                <div className="p-3 rounded-xl bg-surface-900/[0.03]">
                  <p className="text-xs text-surface-900/30 mb-1">{t.horoscope.luckyColor}</p>
                  <p className="text-surface-900 font-semibold">{horoscope.lucky.color}</p>
                </div>
                <div className="p-3 rounded-xl bg-surface-900/[0.03]">
                  <p className="text-xs text-surface-900/30 mb-1">{t.horoscope.auspiciousTime}</p>
                  <p className="text-surface-900 font-semibold">{horoscope.lucky.time}</p>
                </div>
              </div>
            </div>
            ...
            <div className="surface-card p-6">
              <div className="flex items-center gap-2 mb-4">
                <h3 className="font-bold text-surface-900">{t.horoscope.loveRelationships}</h3>
              </div>
              <p className="text-sm text-surface-900/60 leading-relaxed">{horoscope.love}</p>
            </div>

            {/* Career */}
            <div className="surface-card p-6">
              <div className="flex items-center gap-2 mb-4">
                <h3 className="font-bold text-surface-900">{t.horoscope.careerFinance}</h3>
              </div>
              <p className="text-sm text-surface-900/60 leading-relaxed">{horoscope.career}</p>
            </div>

            {/* Health */}
            <div className="surface-card p-6">
              <div className="flex items-center gap-2 mb-4">
                <h3 className="font-bold text-surface-900">{t.horoscope.healthWellness}</h3>
              </div>
              <p className="text-sm text-surface-900/60 leading-relaxed">{horoscope.health}</p>
            </div>
          </div>
        )}
        ...
        {summaryData && !loading && !error && showSummary && (
          <div className="space-y-6">
            ...
            <div className="surface-card p-6 border border-primary-500/20">
              ...
              <p className="text-surface-900/60 leading-relaxed">{summaryData.summary}</p>
            </div>
              ...
              {Object.entries(summaryData.traditions).map(([tradKey, tradData]) => {
                ...
                return (
                  <div key={tradKey} className={`surface-card p-6 border ${colors.border}`}>
                    ...
                    <p className="text-sm text-surface-900/60 leading-relaxed mb-3">{tradData.overview}</p>
                    {tradData.love && (
                      <div className="mb-2">
                        <span className="text-xs text-surface-900/30">{t.horoscope.loveRelationships}:</span>
                        <p className="text-xs text-surface-900/50">{tradData.love}</p>
                      </div>
                    )}
                    {tradData.career && (
                      <div className="mb-2">
                        <span className="text-xs text-surface-900/30">{t.horoscope.careerFinance}:</span>
                        <p className="text-xs text-surface-900/50">{tradData.career}</p>
                      </div>
                    )}
                  </div>
                );
