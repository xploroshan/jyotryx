    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 py-12 fade-in-up">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-surface-50/40 text-sm">{t.numerology.description}</p>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id ? "btn-primary" : "text-surface-50/40 hover:text-surface-50"
              }`}
            >
              {tab.label}
            </button>
        ...
        {activeTab === "name" && (
          <div>
            <div className="surface-card p-6 mb-6">
              <label className="block text-xs text-surface-50/30 mb-2">{t.numerology.enterNameLabel}</label>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={item.label} className="surface-card p-5 text-center">
                      <p className="text-xs text-surface-50/30 mb-1">{item.label}</p>
                      <p className="text-4xl font-bold text-gradient mb-2">{item.num}</p>
                      <p className="text-[11px] text-surface-50/40 leading-relaxed">{item.desc.substring(0, 80)}...</p>
                    </div>
                  ))}
                </div>
                ...
                <div className={`p-4 rounded-xl border ${vc.bg}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-surface-50/30">{t.numerology.rulingPlanet}: {nameResult.rulingPlanet}</span>
                  </div>
                  <p className="text-sm text-surface-50/60 mt-2">{nameResult.suggestion}</p>
                </div>
                  ...
                  <div className="surface-card p-5">
                    ...
                    <ul className="space-y-1.5">
                      {nameResult.strengths.map((s, i) => (
                        <li key={i} className="text-sm text-surface-50/60 flex items-center gap-2">
                ...
                <div className="surface-card p-5">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="text-xs text-surface-50/30 mb-1">{t.numerology.luckyColors}</p>
                      <p className="text-sm text-surface-50/70">{nameResult.luckyColors.join(", ")}</p>
                    </div>
                    <div>
                      <p className="text-xs text-surface-50/30 mb-1">{t.numerology.bestDays}</p>
                      <p className="text-sm text-surface-50/70">{nameResult.bestDaysToUse.join(", ")}</p>
                    </div>
                    <div>
                      <p className="text-xs text-surface-50/30 mb-1">{t.numerology.compatibility}</p>
                      <p className="text-sm text-surface-50/70">{nameResult.compatibility}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {activeTab === "brand" && (
          <div>
            <div className="surface-card p-6 mb-6">
              <label className="block text-xs text-surface-50/30 mb-2">{t.numerology.enterBrandLabel}</label>
              <div className="flex gap-3">
                <input
                  type="text"
                  value={brandInput}
              ...
              <div className="space-y-4">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="surface-card p-5 text-center">
                    <p className="text-xs text-surface-50/30 mb-1">{t.numerology.nameNumber}</p>
                    <p className="text-5xl font-bold text-gradient mb-2">{brandResult.nameNumber}</p>
                    <p className="text-sm text-surface-50/50">{brandResult.vibration}</p>
                  </div>
                  <div className="surface-card p-5 text-center">
                    <p className="text-xs text-surface-50/30 mb-1">{t.numerology.businessScore}</p>
                    <p className="text-5xl font-bold mb-2">
                      ...
                      <span className="text-lg text-surface-50/20">/10</span>
                    </p>
                    <p className="text-sm text-surface-50/50">{t.numerology.ruledBy} {brandResult.planetaryRuler}</p>
                  </div>
                </div>

                {/* Recommendation */}
                <div className="surface-card p-5">
                  <p className="text-sm text-surface-50/70">{brandResult.recommendation}</p>
                </div>
                ...
                <div className="surface-card p-5">
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                      <p className="text-xs text-surface-50/30 mb-1">{t.numerology.bestLaunchDays}</p>
                      <p className="text-sm text-surface-50/70">{brandResult.bestLaunchDays.join(", ")}</p>
                    </div>
                    <div>
                      <p className="text-xs text-surface-50/30 mb-1">{t.numerology.luckyColors}</p>
                      <p className="text-sm text-surface-50/70">{brandResult.luckyColors.join(", ")}</p>
                    </div>
                  </div>
                </div>
              </div>
