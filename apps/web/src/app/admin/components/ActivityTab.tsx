      ...
      <div className="mt-2 space-y-1">
        {Array.from(allKeys).map((key) => (
          <div key={key} className="text-xs flex items-center gap-2">
            <span className="text-surface-50/30 w-20 shrink-0">{key}:</span>
            {prev?.[key] !== undefined && (
              <span className="text-red-400 line-through">{String(prev[key])}</span>
            )}
            {next?.[key] !== undefined && (
              <>
                <span className="text-surface-50/20">&rarr;</span>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <select value={actionFilter} onChange={(e) => { setActionFilter(e.target.value); setPage(1); }}
          className="px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] text-surface-50/60 text-sm">
      ...
      <p className="text-sm text-surface-50/30 mb-4">{total} total actions recorded</p>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${actionColors[log.action] || "text-surface-50/40 bg-white/[0.03]"}`}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-6">
          ...
          <span className="text-sm text-surface-50/40">Page {page} of {totalPages}</span>
