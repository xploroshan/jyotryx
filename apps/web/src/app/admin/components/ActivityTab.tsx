      ...
      <div className="mt-2 space-y-1">
        {Array.from(allKeys).map((key) => (
          <div key={key} className="text-xs flex items-center gap-2">
            <span className="text-ink-500 w-20 shrink-0">{key}:</span>
            {prev?.[key] !== undefined && (
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <select value={actionFilter} onChange={(e) => { setActionFilter(e.target.value); setPage(1); }}
          className="px-3 py-2 rounded-lg bg-black/[0.04] border border-black/[0.10] text-ink-700 text-sm">
      ...
      <p className="text-sm text-ink-500 mb-4">{total} total actions recorded</p>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${actionColors[log.action] || "text-ink-500 bg-black/[0.04]"}`}>
