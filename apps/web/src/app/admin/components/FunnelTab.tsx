    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 items-center mb-6">
        <div className="flex items-center gap-2">
          <label htmlFor="funnel-period" className="text-xs text-white/40">Period:</label>
          <select
            id="funnel-period"
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="funnel-locale" className="text-xs text-white/40">Locale:</label>
          <select
            id="funnel-locale"
      ...
      <h2 className="text-lg font-bold text-white mt-10 mb-4">Cohort retention</h2>
      ...
      <h2 className="text-lg font-bold text-white mt-10 mb-4">Payment failures</h2>
      <p className="text-xs text-white/40 mb-4">
...
function FunnelChart({ data }: { data: FunnelCounts | null }) {
  if (!data) {
    return (
      <div className="surface-card p-6 text-white/30 text-sm">No funnel data.</div>
    );
  }
  ...
  return (
    <div className="surface-card p-6" data-testid="funnel-chart">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-white">Acquisition funnel</h3>
        ...
        <table className="w-full text-xs" data-testid="cohort-grid">
          <thead>
            <tr className="border-b border-white/[0.06]">
              <th className="text-left px-3 py-2 font-medium text-white/40">Cohort</th>
              <th className="text-left px-3 py-2 font-medium text-white/40">Size</th>
              {header.map((h) => (
                <th key={h} className="text-right px-2 py-2 font-medium text-white/40">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {data.map((row) => (
              <tr key={row.cohortWeek} className="border-b border-white/[0.03]">
                <td className="px-3 py-2 text-white/70 whitespace-nowrap">{row.cohortWeek}</td>
                <td className="px-3 py-2 text-white/60 tabular-nums">{row.size}</td>
                {header.map((_, i) => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/[0.06]">
              ...
              <th className="text-left px-4 py-3 text-xs font-medium text-white/40">When</th>
            </tr>
          </thead>
          <tbody>
            {data.map((p) => (
              <tr key={p.id} className="border-b border-white/5">
                <td className="px-4 py-3 text-white">
                  {p.userName}
                  <br />
                  <span className="text-xs text-white/30">{p.userEmail}</span>
                </td>
