  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <label htmlFor="safety-status" className="text-xs text-white/40">Status:</label>
        <select
          id="safety-status"
          ...
          {rows.map((r) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-sm text-white">{r.userName ?? "(deleted user)"}</p>
                  <p className="text-xs text-white/40">{r.userEmail ?? r.userId}</p>
                </div>
              ...
              {r.preview && (
                <div className="mb-3 p-3 rounded-lg bg-white/[0.03] text-sm text-white/80 whitespace-pre-wrap">
                  {r.preview}
                </div>
              )}
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
      {entries.map(([k, v]) => {
        ...
        const tone =
          pct >= 50 ? "text-red-400" : pct >= 20 ? "text-amber-400" : "text-white/60";
        return (
          <div key={k} className="text-[11px] flex items-center justify-between bg-white/[0.02] px-2 py-1 rounded">
            <span className="text-white/50 truncate max-w-[120px]">{k}</span>
