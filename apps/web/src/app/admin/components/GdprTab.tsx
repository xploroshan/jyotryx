  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <label htmlFor="gdpr-status" className="text-xs text-surface-50/40">Status:</label>
        <select
          id="gdpr-status"
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-surface-50/40 truncate">{r.userEmail ?? r.userId}</p>
                  </div>
                  <div className="text-right">
                    {status === "pending" ? (
                      <span className={`text-xs tabular-nums ${
                        overdue ? "text-red-400" : soon ? "text-amber-400" : "text-surface-50/50"
                      }`}>
                        {overdue
                    ...
                    <p className="text-[11px] text-surface-50/30 mt-1">
                ...
                {r.note && (
                  <p className="text-xs text-surface-50/50 mt-3 italic">&ldquo;{r.note}&rdquo;</p>
                )}
                ...
                {status === "pending" && (
                  <div className="flex gap-2 mt-4">
                    ...
                    <button
                      ...
                      className="text-xs px-3 py-1.5 rounded-lg bg-white/[0.05] text-surface-50/60 hover:bg-white/10 disabled:opacity-40"
                    >
                      Reject
                    </button>
                  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="surface-card w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-surface-50">New GDPR request</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-surface-50/40 mb-1">User ID (UUID)</label>
            <input
              value={userId}
          ...
          <div>
            <label className="block text-xs text-surface-50/40 mb-1">Type</label>
            <select
              value={type}
          ...
          <div>
            <label className="block text-xs text-surface-50/40 mb-1">Note (optional)</label>
            <textarea
              value={note}
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={onClose} className="px-4 py-2 rounded-xl surface-card text-sm text-surface-50/60 hover:bg-white/10">Cancel</button>
          <button
            onClick={submit}
