    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3 items-center mb-6">
        <div className="flex items-center gap-2">
          <label htmlFor="ops-window" className="text-xs text-white/40">Window:</label>
          <select
            id="ops-window"
      ...
      {!data ? (
        ...
        <div className="space-y-2">
          ...
          {Object.keys(data.details).length > 0 && (
            <ul className="text-[11px] text-white/40 mt-3 space-y-1">
              {Object.entries(data.details).map(([k, v]) => (
                <li key={k}><span className="text-white/30">{k}:</span> {String(v)}</li>
              ))}
            </ul>
          )}
        </div>
...
function QueueDepthCard({ rows }: { rows: QueueDepthRow[] }) {
  return (
    <div className="surface-card p-6" data-testid="queue-depth-card">
      <h3 className="text-sm font-semibold text-white mb-4">BullMQ queues</h3>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/[0.06]">
              <th className="text-left py-2 pr-4 text-white/40 font-medium">Queue</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Waiting</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Active</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Delayed</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Failed</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Done</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.queue} className="border-b border-white/[0.03]">
                <td className="py-2 pr-4 text-white/70">{r.queue}</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.waiting}</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.active}</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.delayed}</td>
                <td className={`text-right px-2 tabular-nums ${r.failed > 0 ? "text-red-400" : "text-white/60"}`}>{r.failed}</td>
                <td className="text-right px-2 text-white/40 tabular-nums">{r.completed}</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={6} className="text-center py-4 text-white/30">No queues reported</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  return (
    <div className="surface-card p-6" data-testid="error-rate-card">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-white">LLM error rate</h3>
          ...
          return (
            ...
            >
              ...
              {r.topErrors.length > 0 && (
                <ul className="text-[11px] text-white/40 mt-2 space-y-0.5">
                  {r.topErrors.map((t) => (
                    <li key={t.errorCode}>
                      <span className="text-white/60">{t.errorCode}</span>: {t.count}
                    </li>
                  ))}
                </ul>
              )}
    ...
    <div className="surface-card p-6" data-testid={`latency-${title.replace(/\s/g, "-").toLowerCase()}`}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/[0.06]">
              <th className="text-left py-2 pr-3 text-white/40 font-medium">Key</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Count</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">p50</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">p95</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">p99</th>
            </tr>
          </thead>
          <tbody>
            {rows.slice(0, 10).map((r) => (
              <tr key={r.key} className="border-b border-white/[0.03]">
                <td className="py-2 pr-3 text-white/70 truncate max-w-[140px]">{r.key}</td>
                <td className="text-right px-2 text-white/40 tabular-nums">{r.count.toLocaleString()}</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.p50}ms</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.p95}ms</td>
                <td className="text-right px-2 text-white/60 tabular-nums">{r.p99}ms</td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={5} className="text-center py-3 text-white/30">No data</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function CacheHitCard({ rows }: { rows: Array<{ feature: string; total: number; hits: number; hitRate: number }> }) {
  return (
    <div className="surface-card p-6 mt-8" data-testid="cache-hit-card">
      <h3 className="text-sm font-semibold text-white mb-4">Cache hit rate by feature</h3>
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/[0.06]">
              <th className="text-left py-2 pr-3 text-white/40 font-medium">Feature</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Hits</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Total</th>
              <th className="text-right px-2 py-2 text-white/40 font-medium">Hit rate</th>
            </tr>
          </thead>
          <tbody>
            {rows.slice(0, 15).map((r) => {
              ...
              return (
                <tr key={r.feature} className="border-b border-white/[0.03]">
                  <td className="py-2 pr-3 text-white/70 truncate max-w-[220px]">{r.feature}</td>
                  <td className="text-right px-2 text-white/60 tabular-nums">{r.hits.toLocaleString()}</td>
                  <td className="text-right px-2 text-white/40 tabular-nums">{r.total.toLocaleString()}</td>
                  <td className="text-right px-2 text-emerald-400 tabular-nums">{pct}%</td>
                </tr>
              );
            })}
            {rows.length === 0 && (
              <tr><td colSpan={4} className="text-center py-3 text-white/30">No data</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" data-testid="broadcast-modal">
      <div className="surface-card w-full max-w-lg p-6">
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-white/40 mb-1">Audience</label>
            <select
              value={audienceType}
          ...
          {audienceType === "locale" && (
            <div>
              <label className="block text-xs text-white/40 mb-1">Locale code</label>
              <input
                value={locale}
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl surface-card text-sm text-white/60 hover:bg-white/10"
          >
            Cancel
          </button>
          <button
