    ...
    <div>
      ...
      <div className="flex gap-4 mb-6">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, email, or phone..."
          className="flex-1 max-w-md px-4 py-2.5 rounded-xl bg-surface-900/[0.03] border border-surface-900/[0.06] text-surface-900 placeholder-surface-900/20 focus:outline-none focus:border-primary-500/50 focus:ring-1 focus:ring-primary-500/20"
          onKeyDown={(e) => { if (e.key === "Enter") { setUserPage(1); loadUsers(); } }} />
        <button onClick={() => { setUserPage(1); loadUsers(); }} className="px-5 py-2.5 rounded-xl surface-card text-sm text-primary-600 hover:bg-surface-900/10">Search</button>
      </div>
      <p className="text-sm text-surface-900/30 mb-4">{userTotal} users total</p>
        ...
        <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-surface-900/[0.06]">
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Name</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Email</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Role</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Credits</th>
                    <th
                      className="text-left px-4 py-3 text-xs font-medium text-surface-900/40"
                      title="Credits used vs. credits given (current balance + lifetime usage)"
                    >
                      Usage
                    </th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Subscription</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Joined</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-surface-900/40">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id} className={`border-b border-surface-900/5 hover:bg-surface-900/[0.03] cursor-pointer ${selectedUserId === u.id ? "bg-primary-600/10" : ""}`}
                      onClick={() => setSelectedUserId(u.id === selectedUserId ? null : u.id)}>
                      <td className="px-4 py-3 text-surface-900 font-medium">{u.name}</td>
                      <td className="px-4 py-3 text-surface-900/60">{u.email}</td>
                      <td className="px-4 py-3">{roleBadge(u.role)}</td>
                      <td className="px-4 py-3 text-surface-900/60">{u.credits}</td>
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="flex items-center gap-2 min-w-[120px]">
                              ...
                              <div className="flex-1 h-1.5 bg-surface-900/[0.04] rounded-full overflow-hidden">
                                <div
                      ...
                      <td className="px-4 py-3">
                        {u.subscriptionPlan ? (
                          ...
                          <span className="text-xs text-surface-900/20">Free</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-surface-900/30 text-xs">{formatDate(u.createdAt)}</td>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-2">
                          <select onChange={(e) => handleQuickRoleChange(u.id, e.target.value)} value={u.role}
                            className="text-xs px-2 py-1 rounded-lg bg-surface-900/[0.03] border border-surface-900/[0.06] text-surface-900/60">
                  ...
                  {users.length === 0 && (
                    <tr><td colSpan={8} className="px-4 py-8 text-center text-surface-900/30">No users found</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {userTotalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-4">
              ...
              <span className="text-sm text-surface-900/40">Page {userPage} of {userTotalPages}</span>
  ...
  return (
    <div className="mb-6 surface-card p-6" data-testid="churn-risk-sidebar">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-surface-900">Churn risk</h3>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {rows.slice(0, 9).map((r) => (
          <div
            ...
            className="text-left p-3 rounded-lg bg-surface-900/[0.03] hover:bg-surface-900/[0.06] transition-all"
            ...
            >
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  <p className="text-sm text-surface-900 truncate">{r.name}</p>
                  <p className="text-[11px] text-surface-900/40 truncate">{r.email}</p>
                </div>
                <span
              ...
              <div className="flex items-center justify-between mt-2 text-[11px]">
                <span className="text-surface-900/50">
                  {r.reason === "payment_fail"
