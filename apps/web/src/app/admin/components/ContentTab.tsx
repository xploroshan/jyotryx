  ...
  return (
    <div>
      ...
      {error ? (
        ...
        <div className="surface-card p-8 text-center text-ink-500 text-sm">Loading content stats…</div>
      ) : (
        <>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {contentSections.map((section) => (
              <div key={section.title} className="surface-card p-5">
                <h3 className="font-bold text-ink-900 mb-1">{section.title}</h3>
                <p className="text-xs text-ink-500 mb-3">{section.desc}</p>
                <div className="flex items-center justify-between pt-3 border-t border-black/[0.10]">
                  <div>
                    <p className="text-lg font-bold text-gradient">{section.items.toLocaleString()}</p>
                    <p className="text-[10px] text-ink-500">items</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          {contentStats.knowledgeCategories.length > 0 && (
            <>
              <h3 className="text-lg font-bold text-ink-900 mt-8 mb-4">Knowledge Base by Category</h3>
              <div className="surface-card p-6">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {contentStats.knowledgeCategories.map((cat) => (
                    <div key={cat.category} className="flex items-center justify-between p-3 rounded-lg bg-black/[0.04]">
                      <span className="text-sm text-ink-700 capitalize">{cat.category}</span>
