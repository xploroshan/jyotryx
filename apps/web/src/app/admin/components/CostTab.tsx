    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="surface-card p-6">
          <p className="text-xs text-ink-500">MTD LLM Spend</p>
          <p className="text-3xl font-bold text-ink-900 mt-1 tabular-nums">{formatUsd(summary?.mtdUsd ?? 0)}</p>
          {delta && (
        ...
        <div className="surface-card p-6">
          <p className="text-xs text-ink-500">Projected End-of-Month</p>
          <p className="text-3xl font-bold text-accent-400 mt-1 tabular-nums">{formatUsd(summary?.projectionUsd ?? 0)}</p>
          <p className="text-xs text-ink-500 mt-2">Linear projection from MTD ÷ days elapsed × days in month.</p>
        </div>
        <div className="surface-card p-6">
          <p className="text-xs text-ink-500">Today&apos;s Spend</p>
          <p className="text-3xl font-bold text-ink-900 mt-1 tabular-nums">
      ...
      {/* 30-day sparkline */}
      <div className="surface-card p-6">
        <h3 className="text-sm font-semibold text-ink-900 mb-4">Last 30 Days</h3>
        ...
        <div className="flex justify-between text-[10px] text-ink-500 mt-2 tabular-nums">
      ...
      <div className="surface-card p-6">
        <h3 className="text-sm font-semibold text-ink-900 mb-1">Spend Alerts</h3>
        <p className="text-xs text-ink-500 mb-4">
          Hourly cron writes a <code className="text-ink-700">COST_ALERT_TRIPPED</code> row to the
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <label className="block">
            <span className="block text-xs text-ink-500 mb-1">Daily threshold (USD)</span>
            <input
              type="number"
              inputMode="decimal"
              min={0}
          ...
          <label className="block">
            <span className="block text-xs text-ink-500 mb-1">Monthly threshold (USD)</span>
            <input
              type="number"
              inputMode="decimal"
              min={0}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="surface-card p-6">
          <h3 className="text-sm font-semibold text-ink-900 mb-4">Top Features (30d)</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-black/[0.10]">
                <th className="text-left text-[11px] text-ink-500 font-medium py-2">Feature</th>
                <th className="text-right text-[11px] text-ink-500 font-medium py-2">Calls</th>
                <th className="text-right text-[11px] text-ink-500 font-medium py-2">Tokens</th>
                <th className="text-right text-[11px] text-ink-500 font-medium py-2">Cost</th>
              </tr>
            </thead>
            <tbody>
              {byFeature.slice(0, 15).map((r) => (
                <tr key={r.feature} className="border-b border-black/[0.06]">
                  <td className="py-2 text-ink-700">{r.feature}</td>
                  <td className="py-2 text-right text-ink-700 tabular-nums">{r.calls.toLocaleString()}</td>
                  <td className="py-2 text-right text-ink-700 tabular-nums">{r.totalTokens.toLocaleString()}</td>
                  <td className="py-2 text-right text-accent-400 tabular-nums">{formatUsd(r.costUsd)}</td>
                </tr>
              ))}
              {byFeature.length === 0 && (
                <tr><td colSpan={4} className="py-6 text-center text-ink-500">No feature spend in window.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="surface-card p-6">
          <h3 className="text-sm font-semibold text-ink-900 mb-4">Providers &amp; Models (30d)</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-black/[0.10]">
                <th className="text-left text-[11px] text-ink-500 font-medium py-2">Provider</th>
                <th className="text-left text-[11px] text-ink-500 font-medium py-2">Model</th>
                <th className="text-right text-[11px] text-ink-500 font-medium py-2">Calls</th>
                <th className="text-right text-[11px] text-ink-500 font-medium py-2">Cost</th>
              </tr>
            </thead>
            <tbody>
              {byProvider.slice(0, 15).map((r) => (
                <tr key={`${r.provider}|${r.model}`} className="border-b border-black/[0.06]">
                  <td className="py-2 text-ink-700">{r.provider}</td>
                  <td className="py-2 text-ink-700">{r.model}</td>
                  <td className="py-2 text-right text-ink-700 tabular-nums">{r.calls.toLocaleString()}</td>
                  <td className="py-2 text-right text-accent-400 tabular-nums">{formatUsd(r.costUsd)}</td>
                </tr>
              ))}
              {byProvider.length === 0 && (
                <tr><td colSpan={4} className="py-6 text-center text-ink-500">No provider spend in window.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function ForecastCard({ data }: { data: CostForecast | null }) {
  if (!data || data.points.length === 0) {
    return (
      <div className="surface-card p-6 text-ink-500 text-sm" data-testid="forecast-card">
  ...
  const trendTone = data.trend > 0.01 ? "text-red-400" : data.trend < -0.01 ? "text-emerald-400" : "text-ink-500";
  return (
    <div className="surface-card p-6" data-testid="forecast-card">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-ink-900">30-day spend forecast</h3>
...
function WhatIfCard({
  ...
  return (
    <div className="surface-card p-6" data-testid="what-if-card">
      <h3 className="text-sm font-semibold text-ink-900 mb-1">Model-cost what-if</h3>
      <p className="text-xs text-ink-500 mb-4">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-4">
        <label className="block">
          <span className="block text-xs text-ink-500 mb-1">From model</span>
          <select
            value={fromModel}
        ...
        <label className="block">
          <span className="block text-xs text-ink-500 mb-1">To model</span>
          <select
            value={toModel}
        ...
        <label className="block">
          <span className="block text-xs text-ink-500 mb-1">Feature (optional)</span>
          <select
            value={featureFilter}
      ...
      <div className="grid grid-cols-3 gap-4">
        <WhatIfTile label="Window cost today" value={formatUsd(totalCost)} tone="text-ink-900" />
        <WhatIfTile
          label={`If moved to ${toModel}`}
          value={formatUsd(swappedCost)}
          tone={ratio < 1 ? "text-emerald-400" : ratio > 1 ? "text-red-400" : "text-ink-900"}
        />
        <WhatIfTile
          label="MTD impact"
          value={`${mtdDelta >= 0 ? "+" : ""}${formatUsd(mtdDelta)}`}
          tone={mtdDelta < 0 ? "text-emerald-400" : mtdDelta > 0 ? "text-red-400" : "text-ink-700"}
        />
      </div>
    </div>
  );
