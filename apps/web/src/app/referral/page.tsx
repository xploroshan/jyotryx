    ...
    return (
      <div className="relative min-h-screen">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 py-12">
          <div className="surface-card p-8 text-center text-white/40">Loading…</div>
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 py-12 fade-in-up">
          ...
          <p className="text-white/60 mt-2 text-sm max-w-2xl">
            ...
            {status.maxPerReferrer > 0 && (
              <>
                ...
                <span className="text-white/80">{status.maxPerReferrer}</span> times — you have{" "}
        ...
        <section className="surface-card p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-wide text-white/40 mb-1">Your code</p>
              <p className="text-3xl font-mono tracking-widest text-white">{status.code}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => copy(status.code, "code")}
                className="focus-ring px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white text-sm font-medium"
              >
          ...
          <div className="mt-4 p-3 rounded-lg bg-white/[0.03] text-xs text-white/50 break-all">
            {status.shareUrl}
          </div>
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <Stat label="Rejected" value={status.rejectedReferrals} accent="text-white/50" />
          <Stat
            label="Days earned"
              ...
              {status.recent.map((r) => (
                <li key={r.createdAt + r.refereeEmail} className="py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white truncate">{r.refereeName}</p>
                    <p className="text-xs text-white/40 truncate">{r.refereeEmail}</p>
                  </div>
