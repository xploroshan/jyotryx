    ...
    return (
      <div className="relative min-h-screen">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 py-12">
          <div className="surface-card p-8 text-center text-surface-900/40">Loading…</div>
        </div>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-4 py-12 fade-in-up">
        <header className="mb-8">
          ...
          <p className="text-surface-900/60 mt-2 text-sm max-w-2xl">
            When a friend signs up with your code, you both get{" "}
            <span className="text-primary-700 font-semibold">{status.bonusDays} days</span> of
            Premium — full access to every chart, report and AI consultation.
            {status.maxPerReferrer > 0 && (
              <>
                {" "}
                You can claim this bonus up to{" "}
                <span className="text-surface-900/80">{status.maxPerReferrer}</span> times — you have{" "}
        ...
        <section className="surface-card p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-wide text-surface-900/40 mb-1">Your code</p>
              <p className="text-3xl font-mono tracking-widest text-surface-900">{status.code}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => copy(status.code, "code")}
                className="focus-ring px-4 py-2 rounded-lg bg-surface-900/5 hover:bg-surface-900/10 text-surface-900 text-sm font-medium"
              >
              ...
              <button
                ...
                className="focus-ring px-4 py-2 rounded-lg btn-primary text-surface-900 text-sm font-medium"
              >
                Share
              </button>
            </div>
          ...
          <div className="mt-4 p-3 rounded-lg bg-surface-900/[0.03] text-xs text-surface-900/50 break-all">
            {status.shareUrl}
          </div>
        ...
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <Stat label="Rejected" value={status.rejectedReferrals} accent="text-surface-900/50" />
          <Stat
            label="Days earned"
        ...
        <section className="surface-card p-6">
          <h2 className="text-lg font-semibold text-surface-900 mb-4">Recent invites</h2>
              ...
              {status.recent.map((r) => (
                <li key={r.createdAt + r.refereeEmail} className="py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-surface-900 truncate">{r.refereeName}</p>
                    <p className="text-xs text-surface-900/40 truncate">{r.refereeEmail}</p>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-[11px] text-surface-900/30">
