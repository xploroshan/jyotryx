        ...
        >
          {messages.length === 0 ? (
            ...
            messages.map((m) => (
              <View
                key={m.id}
                testID={m.role === 'assistant' ? 'chat-assistant' : 'chat-user'}
                className={`max-w-[85%] mb-2 rounded-2xl px-4 py-2.5 ${
                  m.role === 'user' ? 'self-end bg-accent' : 'self-start bg-surface border border-border'
                }`}
              >
                <Text className={m.role === 'user' ? 'text-fg-onaccent' : 'text-fg'}>
                  {m.content || (sending ? '…' : '')}
                </Text>
              </View>
            ))
